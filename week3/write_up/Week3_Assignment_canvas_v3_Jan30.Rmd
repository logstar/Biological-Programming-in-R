---
title: "Lab Work and Assignment 3 for Biom612 (Week 3) v2"
...
* Try choose(1000,1). Does that make sense?
* Does choose(2,5) have a meaning? 

## Examples using the binomial coefficient

Let's look at a few examples for our analyses.
...
```{r Chunk 6a: binomial distribution test}
...
#Probability using the binomial distribution function dbinom():
...
There is no explicit standard error (SE) function in R. It can be calculated very simply using available functions.

```{r Chunk 6b: Standard Errors CORRECTED}

tcga_lihc<-read.csv("TCGA_LIHC_clinical.csv",  na.string="NA", stringsAsFactors=FALSE, row.names=1)

height_cm<-tcga_lihc$height

mean_height_cm<-mean(height_cm, na.rm=T)
sd_height_cm<-sd(height_cm, na.rm=T)

SE_height<-sd_height_cm/sqrt(length(height_cm))
...
```{r Chunk 11: Confidence interval for OSTP}
...
p<-0.5  #probability of heads,expected value for fair coin toss
...
#Show the sampling-based confidence intervals around the true value of the probability 0.5
...
```{r Chunk 13: Verzani Example 7.3}
...
```{r Chunk 12: one-sample T test, tidy=TRUE}
...
#Add something to the mean to show how the one-sample t-test's pvalue
#diminishes as the test mean moves away from true mean at mu0 (31.63)
...
We can use the same approach to find a confidence interval for $\mu$, the population mean, from the sample mean as we did with the normal distribution:

### Verzani 7.3: Confidence intervals for the population mean, $\mu$

```{r Chunk 14: Verzani Example 7.4}
...
```{r Chunk 15: Verzani Example 7.5}
...
```{r Chunk 16: Verzani Example 7.6}
...
Another alterative is using a statistical test. There are several to choose from, but so far the only one I know of that is specifically designed for symmetry testing is the function symmetry.test() in the lawstats package. This performs well on smaller sets of points. The alternative hypothesis is that the distribution is asymmetric. "How symmetric" depends on the user. Some people accept symmetry tests down to a level of 0.001 or below.

```{r Chunk 18: symmetry.test}
...
```{r Chunk 19: One-sample Wilcoxon Test }

symmetry.test(tcga_luad$cigarettes_per_day)

#is the data symmetric around mu=2 (is mu a good median descriptor?)

wilcox.test(tcga_luad$cigarettes_per_day, mu=2, conf.int=TRUE, conf.level=0.9)


```

# Power analysis for one-sample t-test
```{r Chunk 20: Power analysis for a one-sample t-test }
#see also https://cran.r-project.org/web/packages/pwr/vignettes/pwr-vignette.html

#from class example in the notes:
pwr.t.test( sig.level = 0.05, n=18,
...
Write an R routine/chunk employing the binomial coefficient with the choose() function to produce a plot for the binomial distribution.

You can check your distribution and graph by installing the ExpRep package and using the function S_Local_Limit_Theorem().

## Question 2 {-}

In R, show that (any of the) resulting binomial distributions you generate in Q1 have a mean=np and standard deviation= $\sqrt( np(1-p))$

Use the mean and standard deviation as calculated above, to limit the distribution range (xlim) on your plot. You can also use ylim=c() to include a y-range based on the maximum in your data.

## Question 3 {-}
From Verzani, chapter 7, exercise 7.12.  Also see lecture slides/handout.

The phrasing, “The true value, p, is in the confidence interval with 95%
probability” requires some care. Either p is or isn’t in a given interval. What it means is that if we repeated the sampling, there is a 95% chance the true value is in the random interval. We can investigate this with a simulation. The commands below will find several confidence intervals at once. 

```{r HW Q3 Verani page 186 exercise 7.12}

m = 50 # toss 20 coins 50 times
n = 20
p = 0.5
alpha = 0.10
zstar = qnorm(1 - alpha/2) 
set.seed(2265) #please leave this seed value as is.
phat = rbinom(m,n,p)/n #divide by n for proportions
SE = sqrt(phat*(1 - phat)/n) # compute SE

#We can find the proportion that contains p using
sum(phat - zstar * SE < p & p < phat + zstar * SE)/m

#and draw a nice graphic with
matplot(rbind(phat -  zstar*SE, phat + zstar*SE), + rbind(1:m,1:m),type="l",lty=1)
abline(v=p) # indicate parameter value
```

Do the simulation above. What percentage of the 50 confidence intervals contain the population mean?

## Question 4 {-}
Adapted from Problem 7.20 in Verzani.

We can investigate how robust the T statistic is to changes in the underlying
parent population from normality. In particular, we can verify that if the parent
population is not too skewed or is symmetric without too heavy a tail then the T statistic will still have the t-distribution for its sampling distribution.

The qqplot() function is related to the qqnorm() function. The difference is that qqplot allows us to plot one distribution against another. One way to look at this is there is one distribution along the x-axis, and the second distribution against the y-axis. 

We will simulate one of several distributions in the 'res' variable. We will then plot that simulated distribution against the randomly sampled t-distribution rt(), which is similar to rnorm() in that it samples randomly from the t-distribution.

Here we are graphing the 'res' sample distribution against the t-distribution. Variations away from the linear match on  t-distribution means  that the t-test won't perform well on that distribution,  as the t-distribution is not 'matching' up with the results.
...
for(i in 1:m) {
   x=rbeta(m,5,2) # Change this line only with examples below.
  res[i] = (mean(x) - 0)/(sd(x)/sqrt(n))
  }

qqplot(res, rt(m,df=df)) 
qqline(res, rt(m,df=df))

# examples to try to replace in the line above:
#x = rnorm(n) #original rnorm()
#x=runif(n)-1/2 #r uniform distribution
#x=rt(n, 3) # t distribution
#x=rt (n, 30) #t distribution
#x=rexp(n)-1 #exponential distribution
#x=rbeta(m,5,2) #beta distributions with skew
#x=rbeta(m,2,5)
#x=rbeta(m,5,5)

```

The quantile-quantile plot compares the distribution of the rnorm() sample with a sample from the t-distribution with rt(). You will see that the points are close to linear, as the sampling distribution is the t-distribution.

To test different parent populations against the t-statistic, you can change the line x=rnorm(n) to some other distributions with mean a. For example, try:

* a short-tailed distribution with x=runif(n)-1/2; 
* a symmetric, long-tailed distribution with x =rt(n, 3); 
* a not so long-tailed, symmetric distribution with x=rt (n, 30); 
*  a skewed distribution with x=rexp(n)-1.
* beta distributions: x=rbeta(m,2,5) and two others.

Provide your qqplot and qqline plots of the fits to the t-distribution as in the example above.  Which of the above distributions seem to fit well to the t-distribution? 

## Question 5 {-}

Adapted from Verzani, Chapter 7, Problem 7.18 page 191
Need: library(UsingR)

The data set normtemp (UsingR) contains measurements of 130 healthy, randomly selected individuals. The variable temperature contains normal body temperature. 

a. Does the data appear to come from a normal distribution? If so, find a 90% confidence interval for the mean normal body temperature. 
b. Does it include 98.6 degrees F?
c. What is a significance test's p-value for a 102.1 degrees F temperature against the normtest distribution?
