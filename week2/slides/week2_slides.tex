...
\pdfnote{RV: many interpretations, essentially specifies the distribution of outcomes. Bernoulli RV 0.5. Simulate using coin flip. After we flip, we observed the RV once. Coin is not RV. Some people think of a coin flip has been performed in infinite parallel universes, so that each coin flip is a RV. RV is also interpreted as a function mapping outcome to probability. RV can also be interpreterd as a generator of observations. Statisticians like multiple interpretations of the same thing, and this might be the reason why you can use so many ways to do one thing in R. }
\end{frame}
...
\pdfnote{The probability of observing a result given that some hypothesis is true is not equivalent to the probability that a hypothesis is true given that some result has been observed. data = observation = evidence. p(H) is the prior probability, which estimates the probability of the hypothesis H before the data E. P(H|E) is the posterior probability. P(E|H) is the probability of observing E given H. As a function of E with H fixed, this is the likelihood – it indicates the compatibility of the evidence with the given hypothesis. P(E) is sometimes termed the marginal likelihood or "model evidence". This factor is the same for all possible hypotheses being considered, so this factor does not enter into determining the relative probabilities of different hypotheses.}
...
\pdfnote{In one-sided test, we set critical value z-a with alpha significance level according to the specific tail we are choosing. To calculate p-value, integrate from one end to test statistic according to the hypothesis. In two-sided test, we set critical value with half of alpha. The p-value is one sided p-value times 2. }
\end{frame}
