...
![](R_Command_Line.png "Command Line with Some Examples")
...
![](RStudioSetup.png "The R Studio Environment after creating an R Script file with added annotations for each window")
...
Th3 PDF output from this document represents the output of an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. 
...
#Let's look at the pressure table, using the nice formatting table function "kable" from knitr:

knitr::kable(pressure)

#Plotting in R is very easy, but has lots of options available. Let's start with the most basic:

plot(pressure)

#Let's look at the first fifty colors R offers (for the full range
#of hundreds of colors, just type colors() )
...
plot(pressure, pch=c(1:19), cex=2, col=rainbow(nrow(pressure)), xlab="temp", ylab="p",
     main="Pressure Data \n From R dataset")
...
mydat3<-data.frame( c(10.11, 20.42, 15.15, 35.73), c("4", "5","6", "7"), c("Chihuahua","Shetland Sheepdog", "Dachshund", "Cocker Spaniel"), c("Spike", "Trixie", "Buster", "Basil"), stringsAsFactors = F)
...
* is.finite : Is x a  finite number?
* is.infinite: Is x an infinite result?
...
#subset data across a larger collection. subset() is the easiest way in basic R to get this accomplished.
...
#let's just select the rows where mpg > 30 in mtcars, which is the form that mimics subset.
