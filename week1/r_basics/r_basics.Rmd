---
title: "Biological Programming: R Basics"
...
output:
  ...
  pdf_document:
    ...
    toc_depth: '3'
    highlight: tango
bibliography: r_basics.bib
nocite: |
  @morandat_evaluating_2012, @MarkdownBasics, @UsingMarkdownClass, |
  @Bioconductor2017, @Spreadkeyvaluepair, |
  @BestrmarkdownexampleMarkdownexample2018, |
  @BioconductorInstall, @Pathcomputing2017, |
  @HTML2017, @Markdown2018, @wickham_advanced_2014a
---
...
In this write-up, we are going to set up R and RStudio on your computer. After getting R up and running, we will briefly go through the basic syntax and data types of R programming language. Then, we are going to learn how to use R packages and create R Markdown files. Then, we are going to go through the basic operations of R programming by analyzing a simple biological dataset. Finally, we conclude the required section by discussing the advantages of using R for data analysis comparing to Excel. 
...
> ![](figures/downloadR.png){width=70%}
...
> ![](figures/selectMirror.png){width=50%}
...
> ![R interpreter loop](figures/rInterpreterLoop.png)
...
We can use R interpreter as a calculator.
...
Most of the times, we want to keep the results of some code expressions and use them later. In order to do this, we assign them into variables. 

```{r variables, eval=TRUE}
# variable1 keeps the result of 25 * 25.
# "<-" is the convention for assigning a value to a variable.
# I recommend you NOT to use "=" for variable assignments.
variable1 <- 25 * 25
...
Motivation: keep some analysis procedures somewhere for later usage. 

Sometimes you may find writing R code is tedious because of a lot of copy and paste.
...
Sometimes we want to check a "statement" is either true or false. This is commonly used together with if-else statements and loops introduced in the next two sections. 
...
Operations on a vector are "*broadcast*" to each of its elements, which means performed on each its elements:
...
This behavior of R vectors has **important** consequences, where the type of a vector might be changed after inserting another value of different type without any notification to you. Following is a typical case: if you try to insert a string (or character typed vector) such as "Hi" into a numeric vector, it will change the entire vector into a character typed vector. Your numbers will be "coerced" into characters without any notification. 
...
myNewVec
# Note that the previous number 1, 2, and 3 now become strings "1", "2", and "3", 
# i.e. they are now in character type
```

Some frustrating errors can happen that way, where your calculations are not working because your vector is a character but not a number --  which is why you need the function `class()` to help you figure out what you're working with. R cannot do numerical calculations on a character vector.
...
The `factor` data type is designed to encode categorical values into numerical values, which is often used to perform certain statistical analysis in R, such as logistic regression. We will discuss the use cases of factors in later sessions. For now, you only need to be aware of the differences between a factor and a character typed vector.

An important note on the factor type is that when reading files into R environment or defining a data frame, the default behavior is to turn all character columns into factors, which can be turned off by specifying `stringsAsFactors=FALSE`. This will be used in the following sections. 

Following shows the differences between a factor and a character. We use the function `str()` to display the "structure" of the value inside a variable. 
...
# Let's print out the column and row names
colnames(myDf3)
rownames(myDf3)
```
...
Indexing is the method of selecting values in data frames by indices, and an index is a vector indicating the locations of the selected values in a data frame. 

The syntax of indexing is `myDataFrame[myRowIndex, myColumnIndex]`.

There are mainly four types of indices:
...
# Select all 3 rows and 4 columns
...
To install from Bioconductor, run the following code:
...
2. Set up downloaded package(s) in one or more locations on your computer. These locations are called the "library" of your R environment, which can be checked using function `.libPaths()`. This will be further discussed in the next section about how to use packages. 
...
> ![](figures/rlib.png){width=70%}

Each folder is an installed package with the folder name corresponding to the package name, and the `ggplot2` package is the first one in the list. The content of `ggplot2` is structured within the folder. Following is the screenshot of the files within the `ggplot2` folder:

> ![](figures/ggplot2.png){width=70%}
...
If you want to generate your report in PDF, you will need to install an extra tool on your computer outside of RStudio. This tool, [TeX](https://en.wikipedia.org/wiki/TeX), helps `knitr` to convert R Markdown into PDF. In order to get this tool, click the tiny triangle at the right side of `Knit` option and select `Knit to PDF`, and you will receive an error message in the R Markdown tab. Follow the instructions in the error message carefully to install the TeX distribution. 

> ![](figures/knitToPdf.png)

We will explore the features of R Markdown in the next section. 

# R basic operations

In this section, we will go through the basic operations of R programming language by analyzing a simple and fake biological dataset. The dataset is stored in the file `cell_exp_proliferation.csv` within the lab 1 handout. You can open it with a text editor or Excel.
